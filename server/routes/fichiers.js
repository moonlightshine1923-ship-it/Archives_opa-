...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const reservation = await reserveNumeroBoite(
        connection,
        dossier_id
      );

      const boiteId = reservation.boiteId;
      const numero = reservation.numero;
      ...
      res.status(500).json({
        error: error.message || 'Erreur lors de l’ajout du fichier.'
      });
    } finally {
      if (connection) {
        connection.release();
